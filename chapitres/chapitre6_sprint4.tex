...
\chapter{Sprint 4 : Administration métier, intégration et livraison}

\section{Introduction}

Le quatrième sprint a pour objectif de compléter le coeur documentaire par les modules métier nécessaires à son exploitation, puis de consolider l'ensemble de la solution. Un éditeur de modèles ne suffit pas à lui seul à produire des documents fiables. Il faut également définir les familles de documents, les chartes graphiques, les configurations de vues de tables, les bénéficiaires et les filtres permettant de sélectionner les données métier utilisées dans la génération.
...
La complexité de ce sprint vient de l'interdépendance entre ces modules. Un modèle appartient à une famille, peut être lié à une organisation et peut utiliser une charte graphique. Une famille peut référencer une table de bénéficiaires ou une requête SQL. Une configuration de vue de table décrit les champs visibles, modifiables et utilisés pour la prévisualisation. L'enjeu est donc de proposer une administration cohérente, tout en conservant une architecture maintenable. Cette dernière itération inclut aussi la stabilisation des parcours, les tests de non-régression, la préparation des captures et la mise en cohérence du livrable PFE.
...
\begin{itemize}
    ...
    \item vérifier les parcours complets de génération documentaire ;
    \item préparer la documentation, les captures et la démonstration finale.
\end{itemize}
...
\midrule
US4.6 & En tant qu'évaluateur PFE, je peux suivre un parcours complet et reproductible. & Exécuter les tests, vérifier la sécurité, préparer les captures, documenter l'installation et stabiliser la démonstration.\\
\bottomrule
\end{longtable}
...
\subsection{Administration des modules métier}

Les modules métier permettent d'organiser les vues de tables (TableViewConfigs) par domaine fonctionnel (ex: Ressources Humaines, Formation, Scolarité). Un module regroupe plusieurs vues et définit une table principale.

L'interface du super administrateur permet de créer ces modules, de leur associer une icône, une description et de sélectionner les organisations autorisées à les voir. Cette structure facilite la navigation pour l'utilisateur final en évitant une liste plate de toutes les tables disponibles.

\screenshotplaceholder{sprint4_modules.png}{Administration des modules métier}{Interface de regroupement des vues de tables par domaine}

\subsection{Intégration finale et préparation de la livraison}

La fin du sprint a été consacrée à la validation globale de la plateforme. Cette étape ne consiste pas uniquement à vérifier que chaque écran fonctionne séparément. Elle consiste surtout à rejouer les parcours réels : connexion, sélection du rôle, chargement de l'état de l'éditeur, configuration d'une famille, association d'un modèle, choix d'un bénéficiaire, application des filtres d'année universitaire et prévisualisation du document généré.

Cette consolidation a permis de vérifier la cohérence entre le frontend Angular, les endpoints ASP.NET Core, les services applicatifs, les repositories et les données SQL Server. Les contrôles de sécurité ont également été revus : accès par rôle, portée multi-tenant, validation des requêtes SQL de lecture, filtrage par année universitaire et protection des opérations d'écriture lorsque l'année est clôturée.

Enfin, la préparation de la livraison a porté sur les éléments nécessaires à une soutenance PFE : documentation d'installation, captures d'écran, scénarios de démonstration, vérification des builds et clarification des limites connues. Cette phase donne au projet une forme présentable et exploitable, au-delà du simple fonctionnement technique.
...
\midrule
Regroupement en module & Créer un module et y associer plusieurs vues de tables. & Le module est visible par les organisations sélectionnées.\\
\midrule
...
\midrule
Parcours complet & Générer un document depuis une famille, un modèle, un bénéficiaire et une année universitaire. & Le document rendu contient les données attendues et respecte la charte.\\
\midrule
Validation sécurité & Tester les accès selon les rôles et l'organisation active. & Les données hors périmètre ne sont pas exposées.\\
\midrule
Préparation livraison & Exécuter les builds, préparer les captures et rejouer la démonstration. & Le projet est stable et présentable pour l'évaluation.\\
\bottomrule
\end{longtable}
...
Le sprint 4 a transformé la plateforme en un système documentaire administrable et prêt à être présenté. Les familles, modèles, chartes graphiques, configurations de vues de tables et bénéficiaires sont désormais structurés et exploitables. L'ajout explicite de \texttt{TableViewConfig} constitue un apport important, car il permet d'administrer les données métier de manière configurable sans développer une interface spécifique pour chaque table.

À l'issue de ce sprint, les principales fonctionnalités métier sont présentes et les parcours essentiels ont été consolidés. Le projet dispose ainsi d'une base fonctionnelle complète : authentification, rôles, édition documentaire, administration métier, configuration des vues de tables, gestion des bénéficiaires, filtrage par année universitaire et prévisualisation dynamique.
