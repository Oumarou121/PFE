...
type ObjectColumn = { key: string; label: string; align?: string; width?: string; bold?: boolean };
...
export class DocumentRenderService {
  ...
  buildPreviewHtml(template: TemplateRecord, person: UnknownRecord): string {
    const pages = this.renderDocumentPages(template, person, { mode: 'preview' });
    return `<div class="preview-pages">${this.buildDocumentPagesHtml(template, pages, 'preview-page', { mode: 'preview' })}</div>`;
  }

  renderDocumentPages(template: TemplateRecord, person: UnknownRecord, options: { mode?: RenderMode } = {}): RenderedDocumentPage[] {
    ...
    const displayed = this.applyTemplateHeaderFooterDisplay(pages.length ? pages : [{
      index: 0,
      mode,
      header,
      content: '',
    ...
    return mode === 'print' ? this.sanitizeFinalRenderPages(displayed) : displayed;
  }
  ...
  buildDocumentPagesHtml(
    ...
    const mode = this.getDocumentRenderMode(options);
    const renderClass = mode === 'preview' ? 'document-render document-render--preview' : 'document-render document-render--print';
    return (Array.isArray(pages) ? pages : []).map(page => {
      ...
      const emptyFooterStyle = footerHtml ? '' : ';border-top-color:transparent!important;color:transparent!important;background:transparent!important';
  ...
  printDocPaginated(template: TemplateRecord, person: UnknownRecord): void {
    ...
    const printCSS = `
      ...
      .document-render--print, .sirh-print-page {
      ...
      .document-render--print:last-child, .sirh-print-page:last-child { page-break-after:auto; break-after:auto; }
      .doc-page-header { grid-row:1; box-sizing:border-box; padding:${paddings.header}; font-family:var(--doc-font-body, "Times New Roman", Times, serif); font-size:12pt; line-height:1.6; color:var(--doc-color-text, #111); overflow:hidden; white-space:normal; overflow-wrap:anywhere; }
      .doc-page-body { grid-row:2; min-height:0; box-sizing:border-box; padding:${paddings.body}; font-family:var(--doc-font-body, "Times New Roman", Times, serif); font-size:12pt; line-height:1.6; color:var(--doc-color-text, #111); overflow:hidden; white-space:normal; overflow-wrap:anywhere; }
      .doc-page-footer { grid-row:3; box-sizing:border-box; padding:${paddings.footer}; font-family:var(--doc-font-body, "Times New Roman", Times, serif); font-size:12pt; line-height:1.6; color:var(--doc-color-text, #111); overflow:hidden; white-space:normal; overflow-wrap:anywhere; }
      .var-resolved { color:inherit!important; background:none!important; padding:0!important; border-radius:0!important; font-weight:inherit!important; }
      .var-missing { color:#dc2626; font-style:italic; }
    `;
    ...
    area.innerHTML = this.buildDocumentPagesHtml(template, pages, 'sirh-print-page', { mode: 'print' });
  ...
  getDocumentThemeStyleAttr(template: TemplateRecord): string {
    return Object.entries(this.getDocumentThemeVars(template)).map(([key, value]) => `${key}:${value}`).join(';');
  }

  private buildDocumentContext(template: TemplateRecord, person: UnknownRecord): UnknownRecord {
    ...
    const charter = this.getTemplateGraphicCharter(template);
    const officialName = charter.identity.officialName || organization?.['nom'] || base['nom_etab'] || '';
    return {
      ...base,
  ...
  private resolveAll(html: string, person: UnknownRecord, preview: boolean): string {
  ...
  private resolveObjectTables(html: string, person: UnknownRecord, preview: boolean): string {
    let out = html;
    let guard = 0;
    while (guard++ < 20) {
      const match = /<tr([^>]*)>((?:(?!<\/tr>)[\s\S])*?\{\{#([\w]+):table(?::([\w,\-]+))?\}\}(?:(?!<\/tr>)[\s\S])*?)<\/tr>/i.exec(out);
      if (!match) break;
      ...
      const markerIdx = cells.findIndex(cell => /\{\{#[\w]+:table(?:[:][\w,\-]+)?\}\}/.test(cell.content));
      const columns = this.filterObjectColumns(this.getListObjectColumns(tech), items, this.parseSelectedKeys(rawCols));
      ...
      const cellKeys = cells.map((cell, index) => {
        ...
        const scalarMatches = Array.from(String(cell.content || '').matchAll(/\{\{(\w+)\}\}/g));
        return scalarMatches.map(item => item[1]).find(key => keySet.has(key)) || null;
      });
      const rows = items.map(item => `<tr${trAttrs}>${cells.map((cell, index) => {
        const key = cellKeys[index];
        const content = key ? this.wrapResolved(this.escapeHtml(item[key] ?? ''), preview) : this.resolveScalars(cell.content, person, preview);
        return `<${cell.tag}${cell.attrs}>${content}</${cell.tag}>`;
      }).join('')}</tr>`).join('');
      out = out.replace(fullTr, rows);
    }
    return out.replace(/\{\{#([\w]+):table(?::([\w,\-]+))?\}\}/g, (_match, tech, rawCols) => {
      const items = this.getObjectItems(person, tech);
      const columns = this.filterObjectColumns(this.getListObjectColumns(tech), items, this.parseSelectedKeys(rawCols));
  ...
  private resolveCellExpand(html: string, person: UnknownRecord, preview: boolean): string {
    let out = html;
    let guard = 0;
    while (guard++ < 20) {
      const match = /<tr([^>]*)>((?:(?!<\/tr>)[\s\S])*?\{\{#([\w]+):cell-expand\}\}(?:(?!<\/tr>)[\s\S])*?)<\/tr>/i.exec(out);
      if (!match) break;
      const [fullTr, trAttrs, trInner] = match;
      const cells = this.extractTableCells(trInner);
      const markers = cells.map((cell, idx) => {
      ...
      const rowCount = markerItems.reduce((max, item) => Math.max(max, item.items.length), 0);
      if (!rowCount) {
        out = out.replace(fullTr, `<tr${trAttrs}>${cells.map((cell, idx) => {
      ...
      for (let rowIndex = 0; rowIndex < rowCount; rowIndex += 1) {
        rows += `<tr${trAttrs}>${cells.map((cell, idx) => {
          const marker = markerItems.find(item => item.idx === idx);
          const content = marker ? this.wrapResolved(this.escapeHtml(marker.items[rowIndex] ?? ''), preview) : this.resolveScalars(cell.content, person, preview);
          return `<${cell.tag}${cell.attrs}>${content}</${cell.tag}>`;
        }).join('')}</tr>`;
      }
      out = out.replace(fullTr, rows);
  ...
  private resolveListTags(html: string, person: UnknownRecord, preview: boolean): string {
    return html.replace(/\{\{#([\w]+):(ul|inline)\}\}/g, (_match, tech, mode) => {
  ...
  private resolveScalars(html: string, person: UnknownRecord, preview: boolean): string {
    return String(html || '').replace(/\{\{(\w+)\}\}/g, (match, key) => {
      const value = person[key];
      if (value !== undefined && value !== null && !Array.isArray(value) && typeof value !== 'object') {
        return this.wrapResolved(this.escapeHtml(String(value)), preview);
      }
      return preview ? `<span class="var-missing">{{${this.escapeHtml(key)}}}</span>` : match;
    });
  }

  private getDocumentThemeVars(template: TemplateRecord): Record<string, string> {
    const charter = this.getTemplateGraphicCharter(template);
    const margins = this.getTemplatePageMargins(template);
    const distances = this.getTemplateHeaderFooterDistances(template);
    const orientation = this.getTemplateOrientation(template);
    ...
    return {
      ...
      '--doc-page-bg-image': pageBackground.enabled && pageBackground.image ? this.toCssUrlValue(pageBackground.image, template['updatedAt'] as string || '') : 'none',
  ...
  private getTemplateGraphicCharter(template: TemplateRecord) {
    return template?.id ? this.graphicCharters.getTemplateGraphicCharter(template.id) : normalizeGraphicCharterConfig({});
  }

  private getTemplatePageMargins(template: TemplateRecord): { mt: number; mb: number; ml: number; mr: number } {
    const fallback = this.getTemplateGraphicCharter(template).layout.pageMargins;
    ...
    return { mt: toNum(src?.['mt'], fallback.mt), mb: toNum(src?.['mb'], fallback.mb), ml: toNum(src?.['ml'], fallback.ml), mr: toNum(src?.['mr'], fallback.mr) };
  }

  private getTemplateHeaderFooterDistances(template: TemplateRecord) {
    return normalizeHeaderFooterDistances(template.headerFooterDistances || template['pageHeaderFooterDistances'], this.getTemplateGraphicCharter(template).layout.headerFooterDistances);
  }

  private getTemplateOrientation(template: TemplateRecord): 'portrait' | 'landscape' {
    const fallback = this.getTemplateGraphicCharter(template).layout.orientation;
    const raw = String(template['orientation'] || template['pageOrientation'] || fallback || 'portrait').toLowerCase();
    return raw === 'landscape' ? 'landscape' : 'portrait';
  }

  private getPageSectionPaddings(template: TemplateRecord): { header: string; body: string; footer: string } {
    const margins = this.getTemplatePageMargins(template);
    const distances = this.getTemplateHeaderFooterDistances(template);
    return {
  ...
  private applyTemplateHeaderFooterDisplay(pages: RenderedDocumentPage[], template: TemplateRecord): RenderedDocumentPage[] {
  ...
  private shouldShowSection(mode: 'all' | 'first' | 'even' | 'odd', index: number): boolean {
  ...
  private getSectionDirectionAttrs(template: TemplateRecord, section: 'header' | 'body' | 'footer'): { dir: 'ltr' | 'rtl'; style: string } {
    const dir = this.getTemplateSectionDirection(template, section);
    return { dir, style: `direction:${dir};text-align:${dir === 'rtl' ? 'right' : 'left'};` };
  }

  private getTemplateSectionDirection(template: TemplateRecord, section: 'header' | 'body' | 'footer'): 'ltr' | 'rtl' {
    return template.sectionDirections?.[section] === 'rtl' ? 'rtl' : 'ltr';
  }

  private sanitizeFinalRenderPages(pages: RenderedDocumentPage[]): RenderedDocumentPage[] {
  ...
  private sanitizeFinalRenderHtml(html: string): string {
    ...
    if (!root) return html;
    root.querySelectorAll('.var-resolved, .var-missing').forEach(node => node.replaceWith(doc.createTextNode(node.textContent || '')));
    return root.innerHTML;
  }

  private splitManualPages(html: string): string[] {
    const parts = String(html || '').split(/<!--\s*page-break\s*-->|<hr[^>]*(?:data-page-break|class=["'][^"']*page-break[^"']*["'])[^>]*>/i);
    return parts.length ? parts.map(part => part.trim()).filter(Boolean) : [''];
  }

  private buildObjectTable(items: UnknownRecord[], columns: ObjectColumn[], preview: boolean): string {
    const cols: ObjectColumn[] = columns.length ? columns : items[0] ? Object.keys(items[0]).filter(key => !key.startsWith('_')).map(key => ({ key, label: key })) : [];
    if (!cols.length) return preview ? '<span style="color:#aaa;font-style:italic">(liste vide)</span>' : '';
    ...
    const rows = items.map((row, index) => {
      const rowBg = index % 2 === 1 ? 'background:var(--doc-table-row-alt-bg, #f8fafc)' : 'background:#fff';
  ...
  private getObjectItems(person: UnknownRecord, tech: string): UnknownRecord[] {
    ...
    return Array.isArray(raw) ? raw.filter((item): item is UnknownRecord => !!item && typeof item === 'object' && !Array.isArray(item)) : [];
  }
    ...
    for (const family of families) {
      for (const cls of family.classes || []) {
        ...
        if (variable?.columns?.length) {
          return variable.columns.map(col => ({
  ...
  private filterObjectColumns(
    columns: Array<{ key: string; label: string; align?: string; width?: string; bold?: boolean }>,
    items: UnknownRecord[],
    selectedKeys: string[] = []
  ): ObjectColumn[] {
    const base: ObjectColumn[] = columns.length ? columns : items[0] ? Object.keys(items[0]).filter(key => !key.startsWith('_')).map(key => ({ key, label: key })) : [];
    if (!selectedKeys.length) return base;
    const byKey = new Map(base.map(col => [col.key, col]));
    return selectedKeys.map(key => byKey.get(key)).filter((item): item is { key: string; label: string } => !!item);
  }

  private extractTableCells(rowHtml: string): Array<{ tag: string; attrs: string; content: string }> {
    const cells: Array<{ tag: string; attrs: string; content: string }> = [];
    const re = /<(td|th)((?:\s[^>]*)?|)>([\s\S]*?)<\/\1>/gi;
    let match: RegExpExecArray | null;
    while ((match = re.exec(rowHtml)) !== null) cells.push({ tag: match[1], attrs: match[2], content: match[3] });
    return cells;
  }
  ...
  private toLegacyListItems(value: unknown): string[] {
    ...
    return value.map(item => {
      if (item && typeof item === 'object') return Object.values(item as UnknownRecord).join(' | ');
      return String(item);
    });
  }
  ...
  private normalizeEditorSpacingHtml(html: string): string {
    ...
    root.querySelectorAll('p').forEach(p => {
      ...
      probe.querySelectorAll('br.ProseMirror-trailingBreak').forEach(br => br.remove());
      const hasVisualContent = !!probe.textContent?.replace(/\u00a0/g, ' ').trim() || !!probe.querySelector('img, table, hr, ul, ol, iframe, svg');
  ...
  private stripVariableMarkerStylesHtml(html: string): string {
    ...
    root.querySelectorAll<HTMLElement>('[style]').forEach(node => {
      ...
      const hasAutoVariableColor = /color\s*:\s*#?(2563eb|1d4ed8|7c3aed|f97316)/i.test(styleText) || /background(?:-color)?\s*:\s*#?(eff6ff|f3e8ff|fff7ed)/i.test(styleText);
      if (!hasVariableMarker || !hasAutoVariableColor) return;
      ['color', 'background', 'backgroundColor', 'fontStyle', 'fontWeight', 'padding', 'borderRadius', 'opacity'].forEach(prop => node.style.removeProperty(prop));
  ...
  private normalizeResolvedTableWidthsHtml(html: string): string {
    ...
    root.querySelectorAll('table').forEach(table => {
      ...
      Array.from(firstRow.children).forEach(cell => {
        ...
        const width = String(el.style.width || el.getAttribute('width') || '').trim();
  ...
  private buildOrganizationTemplateVars(org: UnknownRecord): UnknownRecord {
    const raw = org?.['raw'] && typeof org['raw'] === 'object' ? org['raw'] as UnknownRecord : {};
  ...
  private buildTodayTemplateVars(date = new Date(), locale = 'fr-FR'): UnknownRecord {
    const safeDate = date instanceof Date && !Number.isNaN(date.getTime()) ? date : new Date();
    const day = String(safeDate.getDate()).padStart(2, '0');
    const month = String(safeDate.getMonth() + 1).padStart(2, '0');
    const year = safeDate.getFullYear();
    const longFormatter = new Intl.DateTimeFormat(locale, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    const fullFormatter = new Intl.DateTimeFormat(locale, { year: 'numeric', month: 'long', day: 'numeric' });
  ...
  private getPathValue(data: UnknownRecord, path: string): unknown {
    return String(path || '').split('.').reduce<unknown>((acc, key) => acc && typeof acc === 'object' ? (acc as UnknownRecord)[key] : undefined, data);
  }

  private normalizeFilterParamName(value: unknown, fallback = 'filtre'): string {
  ...
  private toCssUrlValue(value: unknown, cacheKey = ''): string {
    ...
    if (!raw) return 'none';
    const finalValue = cacheKey && !/^data:/i.test(raw) && !/^blob:/i.test(raw)
...
class PagePaginator {
  ...
  paginate(contentHtml: string, headerHtml = '', footerHtml = ''): Array<{ header: string; content: string; footer: string }> {
    ...
    if (headerHtml) {
      ...
      this.headerHeight = Math.max(header.offsetHeight, header.scrollHeight, Math.ceil(header.getBoundingClientRect().height));
    ...
    this.footerHeight = Math.max(this.footerReserveHeightPx, footer.offsetHeight, footer.scrollHeight, Math.ceil(footer.getBoundingClientRect().height));
    tempContainer.removeChild(footer);
    tempContainer.remove();

    const availableHeight = Math.max(1, this.contentHeightPx - this.headerHeight - this.footerHeight - this.printSafetyBufferPx);
    const parser = new DOMParser();
    const doc = parser.parseFromString(`<div>${contentHtml}</div>`, 'text/html');
    const content = doc.body.firstElementChild;
    this.distributeContent(content ? Array.from(content.children) : [], availableHeight);
    return this.pages
      .filter(page => this.hasMeaningfulPageContent(page))
      .map(content => ({ header: headerHtml, content, footer: footerHtml }));
  }

  private distributeContent(elements: Element[], availableHeight: number): void {
    let currentPageHtml = '';
    ...
    elements.forEach(el => {
      ...
      if (el.tagName === 'TABLE') {
        const remaining = currentPageHtml.trim() ? Math.max(1, availableHeight - currentPageHeight) : availableHeight;
        this.splitTableElement(el as HTMLTableElement, remaining, availableHeight, tempMeasure).forEach(part => {
          const partHeight = this.measureElementHeight(part, tempMeasure) + this.tableSafetyBufferPx;
          if (currentPageHtml.trim() && currentPageHeight + partHeight > availableHeight) {
      ...
      if (elHeight > availableHeight && this.canSplitTextElement(el)) {
        ...
        this.splitTextElement(el, availableHeight, tempMeasure).forEach(part => {
      ...
      if (lastAddedWasTable && currentPageHtml.trim() && currentPageHeight + elHeight > availableHeight - this.afterTableTextBufferPx) {
        this.pages.push(currentPageHtml);
        currentPageHtml = '';
        currentPageHeight = 0;
        lastAddedWasTable = false;
      }
      if (currentPageHtml.trim() && currentPageHeight + elHeight > availableHeight) {
        this.pages.push(currentPageHtml);
        currentPageHtml = '';
        currentPageHeight = 0;
      }
  ...
  private splitTableElement(tableEl: HTMLTableElement, firstAvailable: number, nextAvailable: number, tempMeasure: HTMLElement): HTMLTableElement[] {
    const thead = tableEl.querySelector('thead');
    const tbody = tableEl.querySelector('tbody');
    const rows = Array.from((tbody?.querySelectorAll(':scope > tr') || tableEl.querySelectorAll(':scope > tr')) as NodeListOf<HTMLTableRowElement>);
    ...
    rows.forEach(row => {
      ...
      const testHeight = this.measureElementHeight(testTable, tempMeasure) + this.tableSafetyBufferPx;
  ...
  private createSplitTableShell(source: HTMLTableElement, thead: Element | null): HTMLTableElement {
    const table = source.cloneNode(false) as HTMLTableElement;
    if (source.hasAttribute('style')) table.setAttribute('style', source.getAttribute('style') || '');
  ...
  private measureElementHeight(el: Element, tempMeasure: HTMLElement): number {
    ...
    return Math.max(wrapper.offsetHeight, wrapper.scrollHeight, clone.offsetHeight, clone.scrollHeight, Math.ceil(wrapper.getBoundingClientRect().height), Math.ceil(clone.getBoundingClientRect().height)) + marginTop + marginBottom;
  }

  private splitTextElement(el: Element, availableHeight: number, tempMeasure: HTMLElement): Element[] {
    const text = String(el.textContent || '');
    const words = Array.from(text.matchAll(/\S+/g)).map(match => ({ start: match.index || 0, end: (match.index || 0) + match[0].length }));
    if (!words.length) return [el.cloneNode(true) as Element];
    const parts: Element[] = [];
    const targetHeight = Math.max(24, availableHeight - this.printSafetyBufferPx);
    let start = 0;
    while (start < words.length) {
      ...
      while (low <= high) {
        ...
        const candidate = this.cloneTextRangeElement(el, words[start].start, words[mid - 1].end);
      ...
      parts.push(this.cloneTextRangeElement(el, words[start].start, words[best - 1].end));
      start = Math.max(best, start + 1);
    }
    return parts;
  }

  private cloneTextRangeElement(el: Element, startOffset: number, endOffset: number): Element {
    const shell = el.cloneNode(false) as Element;
    shell.textContent = String(el.textContent || '').slice(startOffset, endOffset);
    return shell;
  }
  ...
  private canSplitTextElement(el: Element): boolean {
    ...
    return String(el.textContent || '').replace(/\s+/g, ' ').trim().length > 80;
  }
  ...
  private isSkippableLeadingElement(el: Element): boolean {
    ...
    const html = String(el.innerHTML || '').replace(/<br\s*\/?>/gi, '').replace(/\s+/g, '').trim();
  ...
  private hasMeaningfulPageContent(html: string): boolean {
    ...
    return !!probe.querySelector('img, table, hr, ul, ol, iframe, svg') || !!String(probe.textContent || '').replace(/\u00a0/g, '').trim();
  }
  ...
  private applyMeasureContentStyles(root: HTMLElement): void {
    ...
    root.querySelectorAll<HTMLElement>('p').forEach(p => {
      ...
      if (!String(p.textContent || '').trim() && !p.querySelector('img, table, hr, ul, ol, iframe, svg')) p.style.minHeight = '1.6em';
    });
