...
describe('editor normalizers', () => {
  it('normalizes family legacy fields', () => {
    const family = normalizeFamilyRecord({
      ...
      filterCatalogJson: [{ label: 'Departement', type: 'select', staticOptionsText: 'it|IT\nrh|RH' }]
    });
    ...
    expect(family.filterCatalog[0].staticOptions[0]).toEqual({ value: 'it', label: 'IT' });
  });
  ...
  it('normalizes organization charters and table views', () => {
    ...
    expect(organization.graphicCharters[0].config.layout.orientation).toBe('landscape');
  ...
  it('builds document filter params and enabled runtime filters', () => {
    const family = normalizeFamilyRecord({
      id: 'fam_1',
      filterCatalog: [
        { id: 'flt_1', label: 'Age', type: 'number', roles: { admin: true, user: true } },
    ...
    expect(buildDocumentFilterParams({ flt_1: '21' }, family.filterCatalog)).toEqual({
      age: 21,
      filter_age: 21,
      hidden: null,
      filter_hidden: null
